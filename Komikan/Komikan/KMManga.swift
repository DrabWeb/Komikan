...
class KMManga {
    ...
    /// This mangas group for filtering from the sidebar
    var group : String = "";
    
    /// A bool to say if we have already set tmpDirectory
    private var alreadySetTmpDirectory : Bool = false;
    
    /// Extracts this manga to its own TMP folder, and doesnt re-extract if it already has extracted
    ...
    /// Updates the percentFinished by the current page and the page count
    func updatePercent() {
